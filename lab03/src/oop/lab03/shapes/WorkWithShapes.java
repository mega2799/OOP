...
public class WorkWithShapes {
    ...
    private static String shapeDetails(final Shape s) {
        return "Perimeter = " + s.getPerimeter() + ", area = " + s.getSquare();
    }

    public static void main(final String[] args) {
        final Shape circle = new Circle(CIRCLE_RADIUS);

        final Poligon square = new Square(SQUARE_EDGE);
        final Poligon rectangle = new Rectangle(RECT_WIDTH, RECT_HEIGHT);
        final Poligon scaleneTriangle2 = new Triangle(TRIANGLE_EDGE_1, TRIANGLE_EDGE_2, TRIANGLE_EDGE_3);
